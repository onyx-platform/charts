...
function create_config() {
    ...
    if [ -n $container_limits_file ]; then
        . $container_limits_file
    fi
    rm -f $ZK_CONFIG_FILE
    ...
    if [ -n $CONTAINER_CORE_LIMIT ]; then
        echo "zookeeper.nio.numWorkerThreads=$(($CONTAINER_CORE_LIMIT * 2))" >> $ZK_CONFIG_FILE
        echo "zookeeper.commitProcessor.numWorkerThreads=$CONTAINER_CORE_LIMIT" >> $ZK_CONFIG_FILE
    fi
